...
class Student(models.Model):
    ...
    sclass = models.CharField('班级', max_length=20, help_text='例如: 21-03')
...
class QuestionBank(models.Model):
    ...
    title = models.TextField('题目', max_length=500)
    qtype = models.CharField('题目类型', choices=(('单选', '单选'), ('多选', '多选'), ('判断', '判断')), max_length=40)
    a = models.CharField('A选项', max_length=80)
    b = models.CharField('B选项', max_length=80)
    c = models.CharField('C选项', max_length=80)
    d = models.CharField('D选项', max_length=80)
...
class TestquestionBank(models.Model):
    id = models.AutoField('序号', primary_key=True)
    Testid = models.ForeignKey(TestPaper, on_delete=models.CASCADE, verbose_name='试卷序号')
    QuestionBankid = models.ForeignKey(QuestionBank, on_delete=models.CASCADE, verbose_name='题目序号')

    class Meta:
        # 选择这个表之后显示的名字python manage.py makemigrations
        verbose_name = '试卷和题库'
        verbose_name_plural = '试卷和题库'

    def __str__(self):
        return '<%s:%s>' % (self.Testid, self.QuestionBankid)
